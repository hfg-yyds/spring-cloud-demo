...
public class ProducerController {
    ...
    @ApiOperation("发送普通消息")
    public R sendMessage(@PathVariable String info, @PathVariable String topic) {
        Message message = new Message(topic, info.getBytes(StandardCharsets.UTF_8));
        SendResult sendResult = producer.send(message);
        return R.ok().data("sendResult",sendResult);
    }

    @SneakyThrows
    @PostMapping("/sendBatchMessage")
    public R sendBatchMessage(@RequestBody MessageRequest messageRequest) {
